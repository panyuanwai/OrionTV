...
interface PlayerState {
  ...
  videoPlayer: VideoPlayer | null; // expo-video player instance
...
const usePlayerStore = create<PlayerState>((set, get) => ({
  ...
  playEpisode: async (index) => {
    ...
    if (index >= 0 && index < episodes.length) {
      ...
      try {
        // expo-video: 使用 replace 切换视频源，然后 replay
        if (videoPlayer) {
          const newUrl = episodes[index].url;
          videoPlayer.replace({ uri: newUrl });
          videoPlayer.play();
        }
      } catch (error) {
  ...
  seek: async (duration) => {
    ...
    try {
      // expo-video: seekBy 使用秒为单位，duration 参数是毫秒
      videoPlayer.seekBy(duration / 1000);

      const currentTimeMs = (videoPlayer.currentTime || 0) * 1000;
      const durationMs = (videoPlayer.duration || 1) * 1000;
      const newPosition = Math.max(0, Math.min(currentTimeMs + duration, durationMs));

      set({
        isSeeking: true,
        seekPosition: durationMs > 0 ? newPosition / durationMs : 0,
      });
